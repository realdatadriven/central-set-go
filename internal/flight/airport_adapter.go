...
func (a *AirportAdapter) Start(listenAddr string) error {
	...
	for _, s := range a.cfg {
		schemaName := s["arrow_flight"].(string)
		...
		tables, err := discoverTables(a.manager, schemaName)
		if err != nil {
		...
		for _, t := range tables {
			...
			scanFn := makeScanFunc(a.manager, a.mem, schemaName, t.Name, arrowSchema, t.Columns)
...
func discoverTables(db *duckdb.Connector, schema string) ([]tableMeta, error) {
	...
	conn := sql.OpenDB(db)
	defer conn.Close()
	rows, err := conn.QueryContext(context.Background(), q, schema)
	if err != nil {
...
func discoverColumns(db *duckdb.Connector, schema, table string) ([]columnMeta, error) {
	...
	conn := sql.OpenDB(db)
	defer conn.Close()
	rows, err := conn.QueryContext(context.Background(), q, table)
	if err != nil {
...
func makeScanFunc(db *duckdb.Connector, mem memory.Allocator, schemaName, tableName string, aSchema *arrow.Schema, cols []columnMeta) func(ctx context.Context, opts *catalog.ScanOptions) (array.RecordReader, error) {
