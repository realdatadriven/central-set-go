...
func Login(w http.ResponseWriter, r *http.Request) {
	provider := chi.URLParam(r, "provider") // or r.PathValue("provider") in Go 1.22+
	if provider == "" {
		http.Error(w, "provider is required", http.StatusBadRequest)
		return
	}

	cfg, ok := auth.GetProvider(provider)
	if !ok {
		http.Error(w, "unknown provider", http.StatusBadRequest)
		return
	}

	state := generateSecureState() // crypto/rand → base64
	setStateCookie(w, state, provider) // include provider in cookie name or value

	url := cfg.Config.AuthCodeURL(state,
		oauth2.AccessTypeOffline, // or .Online
		// oauth2.SetAuthURLParam("code_challenge", codeChallenge), // if using PKCE
	)
...
func Callback(w http.ResponseWriter, r *http.Request) {
	...
	token, err := cfg.Config.Exchange(r.Context(), code)
	if err != nil {
	...
	userInfo, err := fetchUserInfo(r.Context(), cfg, token)
	if err != nil {
		// ...
	}

	// Create/link session, issue your own JWT, etc.
	// Example: session.Set("provider", provider)
